...
//       alert(`âœ“ Pharmacy billing completed!\nBill ID: ${result.billId}\nTotal: ₹${cartTotal}\nItems: ${cartItemCount}`);
...
export function PharmacyBillingView({
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-2">
        <div>
          <h1 className="text-4xl font-light text-[var(--theme-text)] tracking-tight">
            Pharmacy <span className="font-bold text-[var(--theme-accent)]">Portal</span>
          </h1>
          <p className="text-[var(--theme-text-muted)] text-sm font-medium uppercase tracking-[0.2em] opacity-60">
            Touch Screen Billing & Inventory System
          </p>
        </div>
      ...
      {/* Main Content - Full Width Medicines Grid */}
          ...
          <div className="mb-4 flex gap-2 overflow-x-auto pb-2">
            {categories.map(cat => (
              <button
                key={cat}
                onClick={() => setSelectedCategory(cat)}
                className={`px-3 py-1 rounded text-sm whitespace-nowrap font-semibold transition-colors ${selectedCategory === cat
                  ? 'bg-black text-white border border-[#D4A574]'
                  : 'bg-[#121212] text-[#8B8B8B] border border-[#D4A574] hover:border-[#D4A574]'
                  }`}
              >
      ...
      {/* Floating Cart Icon */}
      {
        cart.length > 0 && (
          <button
            ...
            className="fixed bottom-8 right-8 z-30 flex items-center justify-center bg-[#D4A574] hover:bg-[#C9955E] text-[#0a0a0a] rounded-full shadow-2xl transition-all hover:scale-110"
      ...
      {/* Order Summary Overlay */}
      {
        showOrderSummary && (
          <div className="fixed inset-0 bg-black/70 z-40 flex items-center justify-center p-6">
            <div className="bg-[#121212] border border-[#D4A574] w-full max-w-md rounded-xl overflow-hidden flex flex-col max-h-[85vh] shadow-2xl shadow-black/50">
              {/* Header */}
              <div className="p-4 border-b border-[#D4A574] flex items-center justify-between shrink-0">
                <h2 className="text-white font-bold text-lg">Order Summary</h2>
                <button
                  onClick={() => setShowOrderSummary(false)}
                  className="text-[#8B8B8B] hover:text-white hover:bg-[#1a1a1a] transition-colors rounded-lg p-2"
                  title="Close"
                >
                  <X className="w-5 h-5" />
                </button>
              ...
              <div className="flex-1 overflow-y-auto p-3">
                {cart.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {/* Item Count */}
                    <p className="text-[#D4A574] text-xs font-semibold">{cart.reduce((sum, item) => sum + item.quantity, 0)} {cart.reduce((sum, item) => sum + item.quantity, 0) === 1 ? 'Item' : 'Items'}</p>
